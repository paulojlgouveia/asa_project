...
// 	friend std::ostream &operator<<(std::ostream &out, const Node *node);
	...
	Edge(Node* next, int weight) : _next(next), _weight(weight) { }
	Edge() { }
...
// setters
	...
	Node(int id) : _id(id), _adjSize(0), _pathCost(99999), _visited(false), _parent(NULL) {
...
// 	std::cout << "Johnson" << std::endl;
	deslocationCost = johnson(graph, subsidiaries);
// 	std::cout << graph << std::endl;
